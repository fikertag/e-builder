...
export default function CreatePage() {
  ...
  // Refetch stores and update Zustand after store creation
  async function refetchStoresAndUpdateZustand(
  ...
  const handleFormChange = (
    ...
    if (name === "whyChooseUs") {
      ...
      const reasons = Array.isArray(formData.whyChooseUs)
      ...
      const prev =
        ((formData.integrations && formData.integrations.telebirr) as Partial<{
          number: string;
          name: string;
        }>) || {};
      const telebirr = {
        number:
          key === "number"
            ? value
            : prev && typeof prev.number === "string"
              ? prev.number
              : "",
        name:
          key === "name"
      ...
      setFormData({
        ...
        integrations: {
          ...
          telebirr,
          ...(formData.integrations?.cbe
            ? { cbe: formData.integrations.cbe }
            : {}),
        },
      });
      ...
      const key = name.split(".")[2];
      const prev =
        ((formData.integrations && formData.integrations.cbe) as Partial<{
          account: string;
          name: string;
        }>) || {};
      const cbe = {
        account:
          key === "account"
            ? value
            : prev && typeof prev.account === "string"
              ? prev.account
              : "",
        name:
          key === "name"
      ...
      setFormData({
        ...
        integrations: {
          ...formData.integrations,
          cbe,
          ...(formData.integrations?.telebirr
            ? { telebirr: formData.integrations.telebirr }
            : {}),
        },
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        await refetchStoresAndUpdateZustand(
          setStores,
          setSelectedStoreId,
          setStoreStatus,
          user?.user?.id
  ...
  return (
    <div className="w-full min-h-screen pb-12 md:px-8 flex flex-col gap-10 mx-auto">
      ...
      <div className="w-full max-w-5xl mx-auto p-3 md:p-5 mb-4 flex flex-col items-center">
        ...
        <p className="text-muted-foreground/70 mb-1 mt-3 text-center text-base max-w-2xl">
          Describe your business & products. The AI will fill the form for you
          automatically.
        </p>
          ...
          <div className="flex flex-col gap-y-2 w-full md:col-span-2 mb-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 mb-1">
              <div className="flex flex-col">
                <label className="block text-xs font-normal mb-0 pt-2">
                  Store Name
                </label>
                <input
                  name="storeName"
